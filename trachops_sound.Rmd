...
em_data <- as.data.frame(em_call_type)
...
diet_plot <- ggplot(data = em_data.1, aes(x = PLYBAK, y = mean.class, fill = "black"))
diet_plot <- diet_plot + G4+  C3 + TM3 + Xlab2 + Ex_labs + Ylab2 #all parameters for general data
diet_plot <- diet_plot + geom_errorbar(aes(ymin=asymp.LCL, ymax=asymp.UCL), width=.2) 
